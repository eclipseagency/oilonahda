...
// Snapshots are captured for a fixed period, so the short ranges would all show
// the same figure and read as wrong. Re-add ranges once live Google Ads is ready.
...
const STATUS_STYLE: Record<AdStatus, { ar: string; en: string; cls: string }> = {
  ...
  other: { ar: 'غير معروف', en: 'Other', cls: 'bg-white/10 text-[#999]' },
}
...
export default function AdsPanel({ branch, tr }: { branch: string; tr: (ar: string, en: string) => string }) {
  ...
  // Sum spend per currency; never add EGP and SAR into one number.
  const totals: Record<string, number> = {}
  for (const p of data?.platforms || []) {
    const spend = p.campaigns.reduce((sum, campaign) => sum + campaign.spend, 0)
    totals[p.currency] = (totals[p.currency] || 0) + spend
  }
  const totalLabel = Object.entries(totals).map(([currency, value]) => `${fmt(value)} ${currency}`).join(' · ') || '0'
  ...
  return (
    <div className="space-y-6">
      ...
      {loading && <div className="text-[#999] text-sm">{tr('جار التحميل...', 'Loading...')}</div>}
        ...
        <div key={p.platform} className="bg-[#0D0D0D] border border-[#333] rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#222]">
            <div className="flex items-center gap-2">
              ...
              {p.source === 'snapshot' && <span className="text-xs text-[#999]">{tr('لقطة محفوظة', 'snapshot')}{p.asOf ? ` · ${p.asOf}` : ''}</span>}
              {!p.connected && !p.error && p.source !== 'snapshot' && <span className="text-xs text-[#999]">{tr('غير متصل', 'not connected')}</span>}
            </div>
            <span className="text-sm text-[#999]">{fmt(p.campaigns.reduce((sum, campaign) => sum + campaign.spend, 0))} {p.currency}</span>
          </div>
