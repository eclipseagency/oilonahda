...
function getServiceName(key: string, isAr: boolean): string {
  ...
  const service = services.find(s => s.key === key)
    ?? nahdaServices.find(s => s.key === key)
      ...
      <header className="border-b border-[#333] bg-[#1A1A1A] sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-3 sm:py-4 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          <div className="flex items-center justify-between gap-2 lg:justify-start lg:gap-4 min-w-0">
            ...
            <h1 className="text-base sm:text-lg font-semibold tracking-wide text-[#C9A96E] truncate">{tr('لوحة تحكم أويلو', 'Oilo Admin')}</h1>
            <div className="relative shrink-0">
              <select
                value={viewBranch}
                onChange={e => changeViewBranch(e.target.value)}
                aria-label={tr('تبديل الفرع', 'Switch branch')}
                className="appearance-none cursor-pointer whitespace-nowrap rounded-full ps-3 pe-7 py-1 text-[11px] font-bold tracking-wide [color-scheme:dark] focus:outline-none focus:ring-1 focus:ring-[#C9A96E]"
                style={{ background: 'rgba(201,169,110,0.15)', color: '#C9A96E', border: '1px solid rgba(201,169,110,0.3)' }}
              >
                <option value="al-nahda">{tr('فرع النهضة', 'Al Nahda')}</option>
              </select>
              <svg className="pointer-events-none absolute end-2 top-1/2 -translate-y-1/2 w-3 h-3 text-[#C9A96E]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="bg-[#1A1A1A] border border-[#333] rounded-2xl p-6">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <a href="https://oilo.sa" target="_blank" rel="noopener noreferrer" className="px-3 py-2 bg-white/[0.04] rounded-lg hover:bg-white/[0.08] transition flex items-center gap-2"><span className="text-[#C9A96E]">●</span> {tr('الموقع', 'Website')}</a>
